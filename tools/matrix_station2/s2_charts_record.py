...
#
# NOT PLOTTED (Douglas, 2026-09-14: "Method 3 does not look like it gives a sensible result").
# Chart rev07 carried it as a third ellipse and is kept in chart_versions/; the number is still
# computed and printed because it is a real measurement -- what disqualified it is this cell's
# bracket, the weakest in the matrix (two 82-star fits at 0.6-0.7 " rms, 10 deg of altitude and
# 3.5 min apart, one straight after a slew still settling), not the arithmetic.  Importing a
# quadratic from that bracket imports its faults, and L moved 1.04 " for it.  Optional, so the
# chart tool does not require s2_method3.py to have been run.
HAVE_M3 = bool(glob.glob(os.path.join(OUT, SUB['m3']['100ms'], '**', 'TWOD_RESIDUALS.csv'),
                         recursive=True))
if HAVE_M3:
    D3 = load('m3')
...
if HAVE_M3:  # the same construction as Method 2's, for the printed line only
    STAGE2_SCALE3 = float(np.mean([json.load(open(glob.glob(os.path.join(
        OUT, SUB['m3'][tag], '**', 'distortion_results.txt'), recursive=True)[0],
        encoding='utf-8'))['platescale (arcseconds/pixel)'] for tag, _, _ in TIERS]))
    joint3 = joint_plate_scale(STAGE2_SCALE3, c3[lab3.index('S')], PS)
...
if HAVE_M3:
    print('            Method 3 (computed, NOT plotted since rev08): L = %.3f +- %.3f, '
          'joint scale %.7f, %d observations' % (L3, S3, joint3, len(D3)))
