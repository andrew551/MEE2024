...
| **atmosphere term: ±0.11 ″** | sixteen consecutive zenith nulls with the cell-2 estimator (Method 2, base): rms ±0.109 ″ at airmass 1.001; × airmass^0.73 at the eclipse's 1.064 → ±0.114, carried ±0.11. |
| **budget, provisional** | **L = 1.823 ± 0.096 (stat) ± 0.11 (transfer floor) ± 0.03 (model order) ″**, total σ ≈ 0.15, GR at 0.5 σ, Dittrich et al. 2025's 1.839 ± 0.239 at 0.1 σ; **beside it, not folded: the flat's ±0.2–0.4 ″ (unexplained) and the admission rule's 0.05 ″** |
...
| **dark vs flat on the eclipse field** | four arms, corona on, 138 common stars: neither 1.974, dark only 2.012, **flat only 1.688**, dark+flat 1.811 (residual 0.206 ″, best). The dark moves centroids 37 mas and L +0.04 ″ — protection only. The flat moves every centroid ~250 mas in a field-structured pattern, inward −82 mas at 3–4 R☉, and L **−0.29 ″** on identical stars; mechanism not identified (not the pixel response: 3 mas by injection). Arm of record dark+flat; **±0.2–0.4 ″ carried, unexplained, larger than σ_L**. `s1_caldecomp_fit.py` |
