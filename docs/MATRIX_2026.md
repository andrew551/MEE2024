...
outputs in `F:\MEE_output\station1_record\`. Measured on the 2024-era
...
| **CORRECTION: Station 1 has no Method 1** | Method 1 imports a plate scale, and the refocus put the only calibration **−594 ppm** from the eclipse, so there is no same-day scale that can legitimately be imported. Every "Method 1" figure quoted for this cell (null ±0.30″, eclipse L ≈ +14 to +16″) sizes that impossibility rather than measuring a method, and must not be read as a cell-2 result. The atmosphere term for cell 2 is the **Method 2** null, ±0.101″ |
| **quintic vs septic: measured, and the quintic stays (Douglas, 2026-09-04)** | out of sample the septic does better: null floor 0.126 → 0.105 ″ (−17 %), every eclipse block's residual −1 to −2 % (pooled 0.198 → 0.193 ″), L +0.03 ″. Decision: **the reference of record stays quintic** — higher orders can misbehave near the edges, the difference is not a dominant error, and the published 2024 analysis was quintic. **The +0.03 ″ is reported, not budgeted**: cells 1 and 3 were never tested above their own order, so budgeting it only here would make cell 2 look worse for a reason about which tests were run |
| **calibration matches the published 2024 analysis (Douglas, 2026-09-04)** | dark + flat on the eclipse field, no darks or flats on the zenith fields — held at the published choice so that what changes between the two analyses is the estimator, corona treatment, admission, pooled fit and match gate, not the frames. The flat's 0.3 ″ lever is carried as a sensitivity on a fixed choice |
| **the two-pass match: stage-2 gate fixed, moves nothing** | two pipeline options, both off by default: `distortion_free_scale` (keep the fitted scale under a fixed distortion) and `distortion_fit_tol_initial` (coarse gate, refit, fine gate, refit — a single 3 ″ gate kept 27 of 189 stars because the first fit was pulled by mis-matches inside the 100 ″ rough gate). Stage-2 rms 4 ″ → 0.45–0.50 ″; the joint scales (stage 2's corrected by the pooled S_b) sit **+2 to +4 ppm from Dittrich et al. 2025's 1.847363 ± 0.000013 ″/px** — a scale fitted without L is 46 ppm low and not comparable; the 3 ″ gate removes one G 13 star at 11 R☉ in every block. Pooled L 1.869 → **1.871 ± 0.100**, same 541 rows; the −48 ppm the pooled S_b now read is the deflection stage 2 absorbed and gave back. `stage2_twopass` is the stage-2 of record |
| **the pooled fit, as a tool; every observation a row (Douglas)** | `s1_pooled_fit.py`: 17 parameters (offset, rotation, scale per block; one L). No admission rule — that was the union's need. **L = 1.823 ± 0.096 ″**, 583 obs / 175 stars, residual 0.217 ″; blocks alone 1.98 / 1.72 / 1.78 / 1.82; per-block S_b agree to 3 ppm. Grid (`s1_pooled_grid.py`): admission ≥1/2/3/4 blocks → 1.823 / 1.858 / 1.871 / 1.853 (spread 0.05, the union's 0.21); G ≤ 11/12/13 → 1.829 / 1.823 / 1.823 (no star between G 12 and 13 survives detection); vet none / 3 / 4 / 5 MAD → 1.806 ± 0.234 / 1.815 / 1.823 / 1.826. The 29 single-block stars: median G 11.2, residual 0.51 ″ against 0.26, mean radial +0.07 ″. **σ is nearly flat across the rules (0.093–0.100) because the 87 extra observations are 1.8× noisier and add only 6 % of the information the 496 all-four rows carry** (predicted σ ratio 0.971, measured 0.979); what they buy is radius — in to 2.02 R☉ against 2.52 |
| **scale term: none, by construction** | Method 2 fits S_b with L; σ_L 0.077 free → 0.044 with the scales frozen, so **0.063 ″ of the statistical term is the scale degeneracy** (ρ = −0.58). Cells 1/3 add a term because they import a scale; cell 2 cannot |
| **atmosphere term: ±0.11 ″** | sixteen consecutive zenith nulls with the cell-2 estimator (Method 2, base): rms ±0.109 ″ at airmass 1.001; × airmass^0.73 at the eclipse's 1.064 → ±0.114, carried ±0.11. Named the atmosphere term, as in cells 1 and 3; at the zenith most of it is the estimator's own floor (Leon ±0.12, Leakey ±0.12 on the same statistic) |
| **the chart set and the record folder** | `s1_charts_record.py` → `RECORD/mexico2024/`: deflection (per observation, per star, all-four cross-check), the displacement field in RA/Dec, L-vs-plate-scale covariance against Dittrich et al. 2025, the annotated 0.4 s master, the star table and summary, the night maps, the floor and null tables, the admission grid and the calibration arms. Superseded copies are dated, never deleted — and cell 1's tool now does the same into `RECORD/bruns2017/`. **Revision 2** after Douglas' review: covariance chart in ″/px with the Newton line, 2 σ ellipse and published bands removed; deflection charts without the published line or outlier labels and with a split legend; field chart coloured by witness count (blue 4, green 3, red 2, orange 1); an annotated master for each of the four tiers |
| **the outer radius: 9 → 10 R☉ (Douglas spotted the pile-up)** | 9 R☉ was inherited convention; the frame reaches 10.6. Out to 10 the eclipse field's mean radial residual matches the zenith nulls annulus by annulus; in **10–10.6 it reads +0.201 ± 0.024 ″ against the zenith's +0.043 ± 0.006 ″**, a 6 σ eclipse-day excess in corners where a circle is only 13–24 % inside the frame, so it cannot average over azimuth. Admitting it moves L to 1.667. **Cut set at 10.0 on that comparison, not on L**: +24 % more data, σ_L 0.095 → 0.084, L −0.012 |
| **budget** | **L = 1.804 ± 0.084 (stat) ± 0.11 (atmosphere) ″, total ±0.138**; G ≤ 13, 2–10 R☉, 639 observations of 192 stars, reference gate 0.5 ″. Blocks alone: 1.983 / 1.689 / 1.708 / 1.838 ± ~0.15 (weighted mean 1.805 ± 0.077 with the too-small independent-blocks SE). GR at 0.4 σ; **Newton excluded at 6.7 σ; L/L_Newton = 2.06 ± 0.16**; Dittrich et al. 2025's 1.839 ± 0.239 at 0.15 σ; plate scale 1.8473626 ″/px, 0.2 ppm from published. Reported beside it, not folded: the admission rule's 0.05 ″, the model order's 0.03 ″, the reference gate's 0.01–0.02 ″, the coronal blur's 0.01 ″. The flat's ±0.2–0.4 ″ is withdrawn |
| **coronal blur 10 → 15 px (Douglas)** | re-stacked from raw with the blur σ alone changed: blocks 1.981 / 1.698 / 1.728 / 1.856 against 1.983 / 1.689 / 1.708 / 1.838, spread 0.130 against 0.136, χ²/3 = 0.8 either way, pooled **1.815 against 1.804**. The blocks' scatter is their own, not the model's; a model against no model is the −0.19 / +0.10 per block, its width is +0.011 ″ pooled. 10 px stays; **+0.011 ″ reported beside the budget**. `s1_corona_sigma.py` |
| **the PSF on three instruments (Douglas)** | Leon: isotropic blur, coma with the tail **outward** and strong (skew +0.51 in the corners; moment centroid +150 to +270 mas outward of windowed). Station 1: tangential astigmatism (σ_r/σ_t 0.75) with mild **inward** coma (skew −0.41; moment −150 mas inward). Leakey's Askar 65PHQ: clean (isotropic to 1 %, skew < 0.16, estimators within 40–70 mas, no magnitude slope). Windowing removes the magnitude-dependent leak whichever way the tail points and is neutral where the optic is clean → **recommended default: windowed + annular; fit tolerance 0.2 ″ for cubic on 2600MM-class sensors (Douglas), with the corner-coverage check above cubic or on full frame; cubic stays.** **Applied on v1.4.0-dev 2026-09-05** (Douglas), pinned by `tests/test_defaults.py`; the `eclipse` preset stays the Bruns-reproducing convention and is the way back. **Condition found the same day:** the 2 px window fails on Bruns' field (PSF σ 0.7 px) — residual 0.71 ″ inside 2.5 R☉, L 1.28 ± 0.20 — and a 0.7 px window recovers 1.711 ± 0.101 against the record's 1.777 ± 0.067; **the window must be narrower than the PSF**, stage 1 warns when it is not, and the proper default is a PSF-measured window (F14). Bruns' own night fields added: isotropic, mild inward coma (skew −0.35), moment − windowed −60 to −108 mas, +21 mas/mag. `s1_psf_shape.py` |
| **the scale-free question (Dittrich et al.)** | cell 1 fitted both ways on its own star tables: Method 1 1.764 / 1.794 / 1.718 against Method 2 **1.839 / 1.800 / 1.827** (G ≤ 11 / 10.5 / 13), fitted S −11 / −1 / −14 ppm from the bracketed import (±9); under matched windowing 1.711 against 1.842. Scale-free runs high here too, by +0.006 to +0.109, each under 1 σ; Leon's Method 2 (2.49 ± 1.19) is uninformative. **Existing data cannot settle it beyond 1 σ.** A candidate mechanism is measured: the PSF differs night to day (Station 1's coma −0.41 → −0.18), so the windowed centroid's fixed offset differs between calibration and science field by tens of mas radially — the size that gives a few hundredths in L via ρ ≈ −0.8. The decisive test is Bruns' design: an eclipse-day calibration field away from the Sun (2027) |
| **the PSF, measured (Douglas)** | second and third moments in the radial/tangential frame, 3 zenith + 4 eclipse stacks, G 8–11.5. **Blur is tangential**: σ_r/σ_t 1.00 at the centre → 0.75 (night) / 0.63 (day) in the corners — astigmatism/field curvature, the optic's dominant aberration, not coma. **The tail points inward**: skew_r −0.11 → −0.41 (night), −0.02 → −0.18 (day), tangential skew ≈ 0 (the control). The moment centroid sits ~150 mas inward of the windowed one over the outer field at night with only +18 mas/mag of magnitude dependence (§18.3's +22–31); by day the same offset runs −205 → +84 mas from bright to faint, +145 mas/mag, with the coma if anything smaller — the daytime S/N shrinks the faint stars' footprints out of the tail. Mild inward coma × threshold footprint × daytime S/N = the moment bias. `s1_psf_shape.py` |
| **each block alone (0.5 ″ reference)** | corona, windowed: 1.983 / 1.689 / 1.708 / 1.838 ± ~0.15; weighted mean 1.805 ± 0.077 (SE assuming independent blocks), pooled **1.804 ± 0.088** (clustered) — same centre, the pooled σ is right because the blocks share their stars. χ²/3 = 0.8: no block discordant. Corona vs 2025 stacks, per block: −0.046 / −0.004 / **−0.187** / **+0.096**, pooled −0.026 — real per-block moves of both signs that cancel in the pool. `s1_blocks_alone.py` |
| **the reference's fit tolerance (Douglas)** | 0.3 ″ was undocumented; measured against 0.1 (the 2025 gate) and 0.2 (Bruns' and Leon's): stars per field 959 / 1747 / **2047**, corner stars 28 / 61 / **83**, field-to-field model stability in the corners 316 / 220 / **184 mas**, L against each reference **1.843 / 1.821 / 1.811 ± 0.08** — a 0.03 ″ spread. The reported rms (0.061 / 0.099 / 0.124) is the gate, not the centroids: the corners are noisier at every gate and the tight one removes them. A quintic on this frame needs corner coverage more than a low rms (Douglas' reason). Widened to 0.5 and 1.0 (Douglas): L 1.804 / 1.802 — **a plateau from 0.3 outward**, the 0.1 gate is the outlier (+0.03). The plate scale is the more gate-sensitive quantity: the reference scale drifts −11.7 ppm from the 0.1 to the 1.0 gate and the eclipse joint scale follows (−12.8 ppm); every gate from 0.2 outward is inside the published ±7 ppm, 0.1 is not. At 1.0 the corner rms is 0.252 ″ — mis-matches, not stars. Sensitivity for any gate 0.2–1.0: **0.01–0.02 ″ in L, 5–8 ppm in scale**, reported beside the budget. **Decision (Douglas, 2026-09-05): 0.5 ″ is the reference gate of record** — 105 corner stars per field, and without the 65 stars per field the 1.0 gate adds, whose median residual is 0.50 ″ (the 0.5 population's: 0.09), 70 % fainter than G 14, 27 % in the corners. L 1.811 → 1.804; the 0.3 build stays as `--ref twopass03`. `s1_reference_tolerance.py` |
| **moments on the corona-subtracted stacks (Douglas)** | the four blocks re-stacked from raw with the moment estimator: Lmag **+0.386 ± 0.045 ″/mag** against +0.399 on the 2025 stacks (per block +0.387/+0.332/+0.374/+0.500 vs +0.398/+0.332/+0.362/+0.484). The corona is incidental; the bias is the estimator on this PSF, as in Leon 2026. Windowed on the same stacks: +0.056 ± 0.036. `s1_moments_on_corona.py` |
| **scatter and doubles (Douglas)** | the field chart's 0.31 ″ is per observation, two-axis; per star it is 0.280 ″ (all 192) and 0.173 ″ (the 135 seen in all four blocks) against Bruns' 0.14 — the rest is the day (cloud +28 %, wind), the instrument being quieter than Bruns' at night. Double-star removal was on in every fit (8 flagged of 39 226 zenith matches, none used); the flag sees only companions in the G ≤ 15 catalogue |
| **what differs from the published 2024 analysis** | same 17 zenith stacks, same dark + flat, same quintic, same corrections. Changed: **centroids windowed + annular on both sides** (2024: moments; the eclipse-side convention is the largest lever, +0.36 ″/mag under moments), **per-frame coronal subtraction and the disk occulter**, **four tiers** (the 2025 analysis fitted the 0.4 s block alone, all 123 frames; its 109-frame stack is the 0.25 s block, which starts before second contact), stage 2 constant + isotropic scale with a two-pass gate (2024: linear free at 2.0 ″), **one pooled linear least squares with the scale inside it** (2025 analysis: Nelder–Mead rotation + L on one block, the scale from stage 2, the Mikhailov regression for the covariance), 2–10 R☉ (2024: no cut), the close-in star included, a 4-MAD vet, bootstrap/cluster errors and an atmosphere term. Full table in STEP3 §"What differs from the published 2024 analysis" |
| **against the published analysis (Dittrich et al. 2025 §4.4)** | L 1.811 ± 0.084 against **1.839 ± 0.239**; plate scale 1.847368 ± 0.0000055 against **1.847363 ± 0.000013**; L–scale correlation **−0.786 against −0.783**, which is a property of the field geometry both analyses had to find independently. On the *same* 0.4 s block this chain gives ±0.161 against their ±0.239 — ~1.3× from the fit and error machinery on the same centroids, ~1.1× from windowed, none from the corona (the program's stage 3 was already two-dimensional; an earlier row credited a 2-D fit, wrongly); pooling the other three blocks gives the rest of the 2.8× |
| **correction: the published analysis had no gate problem** | it freed the **linear** terms and ran at a **2.0 ″** fit tolerance. The 20 ″ gate this record described is a property of `constant` mode, which this record had adopted; the two-pass fix restores parity rather than improving on the published work |
| **the close-in star behaves after corona subtraction** | Gaia 2577063532462714368, G 9.7: the paper finds it only in the 250 ms data with ~1 ″ of excess error and excludes it. Here it sits at 2.02–2.03 R☉ in two blocks with residuals 0.28 ″ and 0.30 ″, and removing it moves L by only +0.035 ″ |
| **an external scale for Station 1, from Station 2 (2026-09-07)** | Station 2 is the same train (419.8 mm against 419.3) and has 15 zenith fields plus a left/right bracket. Its night-to-day change is **−723 ppm** on the Bruns/Station 1 pathway, −665 with the bracket's own cubic, −548 at stage 1 — a **175 ppm treatment systematic** against the 71–129 ppm difference from Station 1's −594 that the test needs to resolve, so **inconclusive by 20×** (1.7–3.1 ″ of L against ±0.084). The limit is not statistics (82 stars, 22 ppm) but the 57 ppm frozen-vs-free gap: the distortion changed between epochs, not just the scale. **What survives: four NP101is drop −445 to −723 ppm night to day, two of them with no reducer**, so the cause is in the bare optic and the refocus explanation does not cover it. `docs/STEP3_2026.md`, "Station 2: an external plate scale" |
| **the AVX settle time, a campaign constraint (2026-09-08)** | three measurements on one mount model: Station 2 zenith settles with 28-381 s between captures (ratio 0.91), **Leon 2026 had 25.5 s and still drifts**, Station 2's left bracket had 10 s and reads 1.97. So **~30 s is close to mandatory on a Celestron AVX**, and it buys no photons. Mexico 2024 totality was 267 s: Station 1 slewed **not at all** (0 %), Station 2 twice (60 s, 22 %), Station 3 four times (120 s, **45 %**) -- and Station 3 budgeted 10 s per slew, three times less than the mount needs, which is why that plan could not have worked however well it was coded. All three rows are **Celestron AVX** — Mexico, Portland and Leon all used one — so this is one mount model measured three ways, not three mounts. **For 2027: at most two slews in a short totality on an AVX, or use a harmonic mount (ZWO AM5, which this project has never measured — Joe Izen's Husillos 2026 rig is the first in the data), or do not slew at all as Station 1 did.** , "The Celestron AVX needs ~30 s to settle" |
| **the record charts share one implementation (2026-09-09)** | `tools/record_charts.py` holds the sky frame, the field and covariance charts, the joint-scale arithmetic and the chart writer; all four record chart tools import them. Cut over with **every one of the 39 charts byte-identical** before and after. RA ascending to the right, `joint = stage-2 scale − S × PS`, a unit displacement round-tripping to one arcsec: pinned in `tests/test_record_charts.py`. The structural answer to the four chart divergences of this week |
| **Station 2: one plate scale for both tiers (2026-09-09)** | Douglas: Method 2 split the tiers (2.50 / 0.55 ″) where Method 1 did not — because each tier's free scale, fitted alone, differs by 96 ± 65 ppm (1.5 σ, noise), worth 1.6 ″ of L at 0.0171 ″/ppm. Pool with ONE scale, as cell 2 does across its four blocks: **L = 1.570 ± 0.672 (stat) ± 0.12 (atm) ″**, ρ −0.86, joint scale 1.8657969 ″/px, −54 ppm from the bracket (rev05 printed 1.8671413 on the wrong base; corrected in rev06), 34 observations of 18 stars, no double-star cut. Sharing L alone already brings the tiers' scales to 4.5 ppm apart (−61.2, −56.7); sharing the scale too moves L 0.003 ″. Record set rev05 |
| **Station 2 corrected: an invented radial window (2026-09-08)** | every Station 2 fit was first run at **2–5.5 R⊙**, a bound in no document and not asked for; the record's window is **2–10 R⊙, G ≤ 13** (cell 2 budget). It dropped three of seventeen detections per tier, all at 5.7–6.6 R⊙, worth **0.20 ″ of L and a third of the bar**. Corrected: pooled Method 2 **L = 1.573 ± 0.665 (stat) ± 0.12 (atm) ″** on 34 observations of 18 stars; averaged over the 16 both-tier stars 1.174 ± 0.579; Method 1 on the quadratic-free bracket 2.265 ± 0.284 (right alone 2.037, left alone 2.493). Record set rev03, field charts moved to RA/DEC as cell 2 draws them. `tools/analysis_window.py` + `tests/test_analysis_window.py` now fail any uncited window constant |
| **the Method 1 bar was missing its scale term (2026-09-08)** | each bracket field reports its own plate scale to only **30.5–30.9 ppm on 82–83 stars**, so the imported mean carries ±21.7 ppm = **±0.372 ″ of L** at this field's leverage — larger than the statistical error and never carried. Method 1 is **2.265 ± 0.284 (stat) ± 0.372 (imported scale) ″, GR 1.1 σ away and inside**. It also makes the L−R split of 33.7 ppm only **0.8 σ**: not evidence that either side was disrupted. Splitting each series in half, the left drifts +47.5 ppm (0.9 σ) against the right's +19.6 (0.5 σ) — the left is the noisier field, but trimming its tail is not indicated. `s2_left_halves.py` |
| **the plate scale across totality (2026-09-08)** | both brackets straddle totality in the same order. **Bruns: R8 at 17:42:43 UT → L at 17:44:49, −45.1 ppm. Station 2: right 18:10:55 → left 18:14:30, +33.7 ppm.** Night → day is −466 and −725 ppm, so cold reads a larger scale and warming shortens it; totality's cooling should push the scale back up. Station 2 does, Bruns does the opposite — **the L−R split is not a common thermal drift**, which is why the design takes the mean and bounds the rest by the half-split. Cloud measured in both series: per-frame transparency 0.65–1.11 (right) and 0.53–1.10 (left), one frame each more than 10 % down, worst 33 and 42 % |
| **Station 2's eclipse field (2026-09-08)** | the Station 1 technique verbatim finds **17 stars per tier at 0.42–0.48 ″ rms** in Station 2's 0.100 s and 0.075 s tiers, all beyond 3.2 R⊙. Pooled Method 2 **L = 1.770 ± 1.067 (stat) ± 0.12 (atm) ″** on 15 stars / 28 observations (GR 0.0 σ, Newton 0.8 σ); the 13 stars in both tiers with positions averaged give 1.100 ± 0.958, and the pooled fit on the same 13 gives 1.099 — the averaging changes nothing, the two single-tier stars (one at 3.24 R⊙ reading +1.15 ″) are the whole 0.67 ″. Method 1 tier by tier is 2.501 and 2.545 — the repeatable estimator (the tiers share 13 of their 14 stars but measure them independently, 0.446 ″ per axis apart) — but its scale is good only to 31–57 ppm and this field turns 47 ppm into 0.77 ″, so it reads 2.523 ± 0.314 (stat) ± 0.5–0.9 (scale) ″. **The bracket was also fitted in the wrong convention**: Bruns and Leon both freeze only above-QUADRATIC in their daytime calibration, this froze above-constant. Refitted their way the scale moves 2.0 ppm but the imported quadratic distortion moves L to **2.251 ± 0.306 ″** — the low-order distortion is worth nine times the scale here. Method 2 and the record quotation never touch the bracket and are unchanged. Applying the two bracket sides separately: right alone **1.858 ± 0.331 ″** (0.3 σ from GR, and the best eclipse fit rms), left alone 2.644 ± 0.405, mean 2.251. The left field is worse on four counts (own rms 0.708 vs 0.592 ″, sky scatter 7.35 vs 3.48 % about its trend, worst residual 2.81 vs 2.23 ″, and 14 % worse when imported) but is **not** trailed and the pointings are symmetric — and the 33.7 ppm L−R split is SMALLER than Bruns' own 45.0. The difference is leverage: **0.0165 ″/ppm here against Bruns' 0.0054**, because his innermost star is at 1.00 R⊙ and this field's at 3.21. Same bracket quality, three times the damage; the mean stays the estimator. Not a measurement of L; a demonstration, plus Station 2's own atmosphere null **±0.12 ″**, the same as Station 1's. `RECORD/mexico2024st2/` rev02; `docs/STEP3_2026.md`, "Station 2's eclipse field" |
| **what `double_star_cutoff` should be (2026-09-08)** | measured, not assumed. Blending is directional, so project each star's residual onto its neighbour: on **1712 zenith stars, 294 with a neighbour inside 60 ″, six epochs each at 0.167 ″ rms**, the pull toward the companion is **+0.39 to +0.53 ″ inside 10 ″** (2.5–3× the whole residual rms) and **−0.01 to −0.03 ″ beyond it**, with the perpendicular control flat throughout. It follows the flux-weighted prediction with **slope +1.00 inside 4 ″**, degrading from 8 ″ and gone by 14 ″ — a merge radius of ~2.7× the 3.7 ″ PSF FWHM. **The 10 ″ default sits exactly at the transition and is well placed**; what is wrong is the SHAPE of the rule, since at fixed separation the bias scales with the companion's brightness: cell 2's closest pair, 0.63 ″ with ΔG 4.4, is predicted to bias by 0.010 ″ and is discarded, while a 9.92 ″ pair with ΔG 3.1 biases by 0.52 ″ and barely survives. Rule the data supports: cut on predicted shift `sep × f2/(f1+f2)` above ~0.05 ″, within ~2.7× FWHM. `docs/STEP3_2026.md`, "What `double_star_cutoff` should be" |
| **the cut on the companion's brightness, calibrated (2026-09-09)** | Douglas: cut on the companion's magnitude as well as its distance. `b = sep × f2/(f1+f2)` IS the bias in arcsec (slope +1.00), so it can be compared with the field's noise where a radius cannot. Calibration on the zenith sample: the merge radius does **not** depend on the primary's brightness (both bright and faint switch off at 10–14 ″), so magnitude enters once, through the flux ratio; the threshold is loosely determined (0.02–0.20 ″ all remove 36–47 of the 48 stars inside 12 ″) and should be a stated fraction of the field's residual rms — 0.05 ″ is a third of the zenith's 0.167 ″. Cost: **Station 1 10 → 9, Station 2 2 → 2, Leon 0 → 0, Bruns 0 → 0**. The rules differ on one star in the whole matrix — cell 2's 0.63 ″ pair with a 4.4-magnitude-fainter companion, predicted bias 0.010 ″, which only the radius rule discards. L: record 1.804, radius rule 1.830, blend rule 1.835 ″ |
| **double-star removal inoperative offline (2026-09-08, F31)** | the stage-2 cut has removed nothing on any offline reduction: the offline provider returns the flagged stars, the caller expects their companions. Checked directly against the catalogue: **Bruns 0 of 27, Leon 0 of 42** (its union tool used `is_double` and is clean), **Station 1 10 of 192** — refit without them **1.830 ± 0.091 against the record's 1.804 ± 0.084, +0.026 ″, 0.3 σ, not applied** — Station 2 2 of 17. `docs/STEP3_2026.md`, "Double-star removal has been inoperative" |
| **OPEN, matrix-wide: do scale-free fits run high?** | the paper notes 1.839 (Station 1), 1.86 (Bruns eclipse-field-only) and 1.83 (Lick 1922) clustering within 0.03 ″ and suspects a systematic in scale-free analysis. **This reduction is scale-free and lands at 1.823, 4.1 % above GR**, while cell 1 (imported scale) gives 1.720 and cell 3 gives 1.914. Zenith nulls show no bias in the estimator (−0.05 ± 0.11 ″) but carry no Sun. **The test: refit cells 1 and 3 scale-free and see whether they move up ~5 %** |
| **error bars checked (Douglas)** | OLS 0.080 ″ / 2.7 ppm; cluster-robust on star **0.095 ″ / 3.5 ppm**; star bootstrap 0.095 ″ / 3.6 ppm. σ_L rises 0.051 → 0.080 when the scale is freed (ρ = −0.82) and → 0.095 with clustering. The revision-2 chart's 5 × 10⁻⁶ ″/px scale error was the OLS value and was too small |
| **the pooled fit** (Douglas) | every block observation its own row, per-block offset/rotation/scale, one shared L: 541 rows, 142 stars, **L = 1.869 ± 0.098 ″** (bootstrap over stars; the formal ±0.077 wrongly treats rows as independent). Supersedes the union's 1.987 ± 0.105 as the estimator of record; the union's per-star averaging had folded block-to-block scale differences into the residuals |
| the floor at the eclipse altitude | residual ∝ airmass^0.73 across the five measured floors (Leon alone: ^0.80); the eclipse at airmass 1.064 against the calibration's 1.001 predicts 0.078 → **0.082 ″, +0.004 ″**, an upper bound. No mosaic needed |
| corona and integration time | the t^+0.22 ordering falls to t^+0.11 after the corona subtraction, inside four-point noise. Coma cannot order by exposure (it is a function of field position); the corona could, and removing it removed the ordering |
| **dark vs flat on the eclipse field: no systematic** | four arms, corona on. On the 144-star common science set with no vet the arms span 0.16 ″ (neither 1.662, dark 1.684, flat 1.746, dark+flat 1.820, ±0.23 each) — agreement. The ~250 mas per-star shift the flat produces is **re-drawn measurement noise**: ρ = +0.70 with magnitude (56 mas bright quartile, 461 faint), ρ ≈ 0 with the flat's value and gradient, nearest-neighbour correlation +0.08, identical alignment, computed pixel response 1.4 mas. ΔL is 0.3–0.6 σ of an isotropic-noise null. **The earlier −0.29 ″ and its ±0.2–0.4 ″ carry are withdrawn**: they compared arms on each arm's own vetted subset. `s1_caldecomp_fit.py`, `s1_flat_mechanism.py` |
