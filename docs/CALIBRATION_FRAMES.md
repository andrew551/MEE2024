...
| Mexico 2024 eclipse field — flat | **kept (the published 2024 choice); no systematic to carry** | The flat-fielded re-stack's centroids differ from the un-flatted one's by ~250 mas per star, but that is **re-drawn measurement noise**: it tracks brightness (ρ = +0.70; 56 mas in the bright quartile, 461 in the faint), not the flat's own value or gradient (ρ ≈ 0), and neighbouring stars move independently (nearest-neighbour correlation +0.08). The pixel response is 1.4 mas computed and 2.4–3.3 mas injected. ΔL between the arms is 0.3–0.6 σ of an isotropic-noise null; the −0.29 ″ once reported here came from comparing arms on each arm's own vetted subset. `s1_flat_mechanism.py` |
