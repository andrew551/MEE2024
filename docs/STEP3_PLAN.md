...
`docs/REFRACTION_2026.md` (§10–§17) and `F:\MEE_output\CAL_PILEO_STEP2.md`;
...
|---|---|
| raw frames | `G:\Leon Aug 2026\2026-08-12\Eclipse\SCI_ladder\` — tiers `0.1s/ 0.3s/ 0.6s/ 1.2s/` + `discard/` (one ~50 ms default-setting frame). **G: only for Leon 2026**; the `I:` and `J:\Eclipse data` copies of *this campaign* are superseded backups, never its analysis source. (That is campaign-scoped: `I:` also holds the primary `Don Bruns 2024`, `Kenneth Carrell 2024`, `Leakey 2024` and `Papers`, which have no G: counterpart and are legitimate sources.) Folder organisation is truth; EXPTIME headers lie on block-first frames after a SET EXPOSURE change (verify by sky level). |
| CAL_piLeo result to import | `F:\MEE_output\leon2026\cal_pileo_step2\canonical_16f_night2refs\` — **2.2054043 ″/px**, 74 stars, rms 0.5318″, `observation_time 18:29:35`, HC0 21.6 ppm (quote HC3-class ~25 ppm) |
| zenith references | the **six 08-12 files only** (night-2; the two-night 12-file set is superseded — the telescope was transported between night 1 and eclipse day and measurably changed: tilt dipole ×2, +197 ppm scale). `F:\MEE_output\Claude Code\HANDOFF_zenith_cubic\inpipeline_windowed\08-12_Z*.txt`; d(3000) carried forward = **3.0297″**, not 3.1048 |
...
- `analysis/vertical_test.py` is superseded (90° rotation error); use the alt-az affine
  (`m3_maps.py`).
- The venv, never system Python. Write only to `F:\MEE_output` and the
  repo.
